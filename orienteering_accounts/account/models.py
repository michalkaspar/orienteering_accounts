...
from django.db.models import Sum, QuerySet, Q, Max
...
class Permission(BaseModel):
    ...
    code = models.CharField(
        max_length=256, verbose_name=_("Kód"), db_index=True, unique=True
    )
...
class Role(BaseModel):
    ...
    permissions = models.ManyToManyField(
        "account.Permission", related_name="roles", blank=True, verbose_name=_("Práva")
    )
...
class Account(PermissionsMixin, AbstractBaseUser, BaseModel):
    ...
    registration_number: str = models.CharField(
        max_length=7, verbose_name=_("Registrační číslo"), unique=True, db_index=True
    )
    gender: Gender = models.CharField(
        max_length=5, choices=Gender.choices, verbose_name=_("Pohlaví")
    )
    ...
    init_balance = models.DecimalField(
        decimal_places=2, max_digits=9, default=Decimal(0)
    )
    ...
    clubroom_chip_number = models.CharField(
        max_length=255, blank=True, verbose_name=_("Číslo čipu od klubovny")
    )
    ...
    def balance(self) -> Decimal:
        return self.init_balance + Decimal(
            str(
    ...
    def balance_without_entries(self) -> Decimal:
        return self.init_balance + Decimal(
            str(
                ...
                .exclude(origin_entry__isnull=False)
                .aggregate(balance=Coalesce(Sum("amount"), Decimal(0)))["balance"]
            )
        )
    ...
    @property
    def membership_paid_to_date(self):
        return self.transactions.filter(
            purpose=Transaction.TransactionPurpose.CLUB_MEMBERSHIP,
        ).aggregate(max_date=Max("period__date_to"))["max_date"]
    ...
    def get_transactions_descendant(
        self, filters: Q = None
        ...
        other_debts_by_origin_entry_id: dict[int, typing.List[Transaction]] = (
            defaultdict(list)
        )
        ...
        for other_entry_transaction in qs.filter(
            ...
            other_debts_by_origin_entry_id[
                other_entry_transaction.origin_entry_id
            ].append(other_entry_transaction)
        ...
        for transaction in qs.exclude(
            ...
            if (
                transaction.purpose == Transaction.TransactionPurpose.ENTRY
                and transaction.other_debts
            ):
                others_sum = sum(
                    (d.amount for d in transaction.other_debts), Decimal("0")
                )
                transaction.total_with_others = (
                    transaction.amount or Decimal("0")
                ) + others_sum
    ...
    def get_transactions_other_sum(self) -> Decimal:
        ...
        result = (
            self.transactions.filter(~Q(created__year=datetime.now().year))
            .exclude(purpose=Transaction.TransactionPurpose.CLUB_MEMBERSHIP)
    ...
    def club_membership_payment_amount(self):
        return (
            Decimal("2000.00").quantize(Decimal(1))
            if self.is_adult
            else Decimal("5000.00").quantize(Decimal(1))
        )
    ...
    def is_adult(self):
        ...
        born_year = (
            self.born_year + 2000
            if self.born_year < current_year - 2000
            else self.born_year + 1900
        )
    ...
    def send_rest_payment_info_email(self):
        ...
        html_content = render_to_string(
            "emails/account_rest_payment_info.html", context
        )
    ...
    def send_debts_payment_info_email(self):
        ...
        html_content = render_to_string(
            "emails/account_debts_payment_info.html", context
        )
    ...
    def send_entry_rights_removed_info_email(self):
        ...
        html_content = render_to_string(
            "emails/account_entry_rights_removed_info.html", context
        )
    ...
    def send_entry_rights_restored_info_email(self):
        ...
        html_content = render_to_string(
            "emails/account_entry_rights_restored_info.html", context
        )
    ...
    def send_unregistered_membership_payment_email(self):
        context = {"account": self}

        html_content = render_to_string(
            "emails/unregistered_membership_payment.html", context
        )

        email_utils.send_email(
            recipient_list=settings.ACCOUNT_CREATED_EMAILS_SEND_TO,
            subject=f"Příspěvek od neregistrovaného člena - {self.full_name} {self.registration_number}",
            html_content=html_content,
        )
    ...
    def add_to_google_workspace_group(
        self, group_email: str = settings.GOOGLE_GROUP_MEMBERS
    ):
    ...
    def remove_from_google_workspace_group(
        self,
        group_email: str = settings.GOOGLE_GROUP_MEMBERS,
        email: str = None,
        email2: str = None,
    ):
    ...
    def process_bank_transaction(
        ...
        oris_registered_numbers: set = None,
    ):
        ...
        try:
            if bank_transaction.entryDetails.transactionDetails.relatedParties.counterParty:
                sender_name = (
                    bank_transaction.entryDetails.transactionDetails.relatedParties.counterParty.name
                    or ""
                )
        ...
        try:
            if bank_transaction.entryDetails.transactionDetails.remittanceInformation:
                ...
                receiver_note = (
                    remittance.unstructured or remittance.originatorMessage or ""
                )
        except AttributeError:
            pass
        ...
        if variable_symbol and amount > 0:
            ...
            current_year = datetime.now().year
            club_membership_variable_symbol_prefixes = (
                str(current_year),
                str(current_year - 1),
            )

            if any(
                prefix in variable_symbol
                for prefix in debts_variable_symbol_prefixes
                + club_membership_variable_symbol_prefixes
            ):
                variable_symbol = variable_symbol.strip().lstrip("0")
                registration_number = variable_symbol[4:]

                account = cls.all_objects.filter(
                    registration_number=f"TZL{registration_number}"
                ).first()
                ...
                if account:
                    ...
                    if variable_symbol.startswith(
                        ...
                        if account.removed_from_google_workspace:
                            ...
                            account.add_to_google_workspace_group()
                            account.save(
                                update_fields=["removed_from_google_workspace"]
                            )

                        # Check if user is registered in ORIS for the current season
                        if (
                            oris_registered_numbers
                            and account.registration_number
                            not in oris_registered_numbers
                        ):
                            logger.warning(
                                "Club membership payment received from user not registered in ORIS",
                                extra={
                                    "account": account,
                                    "registration_number": account.registration_number,
                                },
                            )
                            account.send_unregistered_membership_payment_email()

                        logger.info(
                            "Processed and charged entry bank transactions",
                            extra={"account": account, "amount": amount},
                        )
                    else:
                        transaction_kwargs.update(
                        ...
                        logger.info(
                            "Processed and charged debts bank transactions",
                            extra={"account": account, "amount": amount},
                        )
    ...
    def ranking(self) -> typing.Optional[UserRanking]:
        ranking_db_client = redis.StrictRedis.from_url(
            f"{settings.REDIS_LOCATION}/{settings.REDIS_RANKING_DB_NUMBER}"
        )
        ranking = ranking_db_client.hget(
            settings.REDIS_CLUB_USER_RANKING_KEY, self.registration_number
        )
...
class Transaction(BaseModel):
    ...
    period = models.ForeignKey(
        ...
        verbose_name=_("Období"),
    )
    amount = models.DecimalField(
        decimal_places=2, max_digits=9, verbose_name=_("Částka")
    )
    purpose = models.CharField(
        max_length=50,
        choices=TransactionPurpose.choices,
        default=TransactionPurpose.CLUB_MEMBERSHIP,
        verbose_name=_("Účel transakce"),
    )
    note = models.TextField(verbose_name=_("Poznámka"), blank=True, default="")
    origin_entry = models.ForeignKey(
        "entry.Entry", on_delete=models.SET_NULL, null=True, related_name="transactions"
    )
    origin_bank_transaction = models.ForeignKey(
    ...
    author_name = models.CharField(
        max_length=255, verbose_name=_("Autor změny"), blank=True, default=""
    )
    ...
    def is_event(self):
        return self.purpose in [
            self.TransactionPurpose.ENTRY,
            self.TransactionPurpose.ENTRY_OTHER,
        ]
...
class BankTransaction(BaseModel):
    ...
    remote_id = models.CharField(
        max_length=255, verbose_name=_("ID transakce"), unique=True, db_index=True
    )
    ...
    purpose = models.CharField(
        max_length=50,
        choices=BankTransactionPurpose.choices,
        default=BankTransactionPurpose.DEBTS,
        verbose_name=_("Účel transakce"),
    )
    sender_name = models.CharField(
        max_length=255, verbose_name=_("Jméno odesílatele"), blank=True, default=""
    )
    receiver_note = models.TextField(
        verbose_name=_("Poznámka pro příjemce"), blank=True, default=""
    )
